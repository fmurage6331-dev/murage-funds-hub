...
1. Check the hosted project's migration ledger **before** applying `supabase/migrations/20260924090000_bot_membership.sql` to **cbfciiehhpkpdtxrltrc**. Several historical `20260918103...` files in this repository are overlapping schema snapshots (for example, one recreates `loan_repayments` after the earlier priority migration); a blind replay of every file into a fresh database fails. Do not run `supabase db reset`/`supabase db push` against the live project without reconciling its ledger and taking a backup. Apply the *new* migration only after the prerequisites are present. It adds RLS/unique indexes, restores the `has_role` grant needed by authenticated RLS, and changes member foreign keys to `profiles.id` so phone-only members do **not** need an `auth.users` row. Resolve any duplicate phone numbers or M-Pesa references surfaced by the unique indexes rather than silently merging members. Only the admin can read pending applications in the web app; the callback and approval workflows use the Edge Function service role after authorization.
...
AT receives HTTP **200** (even when the provider/database is unavailable); the answer is sent via the AT Messaging API, not the webhook response body. Unauthorized callbacks also receive a generic 200 and are ignored. Watch the Supabase Edge logs for error details without logging phone message bodies or secret values. The helper and webhook have local mock-provider/PostgREST tests: `npx deno test --no-config --allow-env supabase/functions/_shared/africastalking.test.ts supabase/functions/whatsapp-bot/index.test.ts`. The phone-only SQL/RLS path has an isolated PostgreSQL/WASM test (no external credentials): `npx deno test --no-config --allow-read --allow-env supabase/tests/bot-membership.test.ts`. The SQL test exercises the earlier runnable migration path plus the new migration, not the overlapping historical snapshots or the hosted project. Neither test replaces a live Africa's Talking smoke test.
