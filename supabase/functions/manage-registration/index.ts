...
function jsonResponse(body: { success: boolean; error?: string; warning?: string }, status: number): Response {
...
function parseBody(value: unknown): ReviewRequest {
  ...
  if (data.action === "reject" &&
    (typeof data.reason !== "string" || data.reason.trim().length < 5 || data.reason.trim().length > 500)) {
...
async function verifiedAdmin(request: Request): Promise<string> {
  ...
  const roles = await dbResult(
    "admin role",
    adminClient().from("user_roles").select("id").eq("user_id", user.id).eq("role", "admin").limit(1),
  );
...
async function deliverWelcome(application: Application): Promise<void> {
  ...
  if (application.email) {
    ...
    if (error || !data?.properties?.action_link) throw new Error("Unable to generate password setup link");
  ...
  await dbResult(
    ...
    adminClient().from("pending_registrations").update({
      invite_sent: true,
      invite_sent_at: new Date().toISOString(),
    }).eq("id", application.id).eq("status", "approved"),
  );
}
...
async function approve(application: Application, adminId: string): Promise<{ warning?: string }> {
  ...
  if (application.email) {
    const existing = await dbResult(
      "find email profile",
      adminClient().from("profiles").select("id,email,phone_number,is_anonymized")
        .ilike("email", application.email).limit(1).maybeSingle(),
    );
    if (existing) {
      if (existing.is_anonymized || (existing.phone_number && existing.phone_number !== application.phone_number)) {
        throw new Error("This email already belongs to a different member");
      }
      authId = existing.id;
    } else {
      ...
      const created = await adminClient().auth.admin.createUser({
        email: application.email,
        email_confirm: true,
        user_metadata: { full_name: application.full_name, consent_given: true, consent_version: "1.0" },
      });
  ...
  try {
    await dbResult("approve bot registration", adminClient().rpc("approve_bot_registration", {
    ...
    if (newlyCreatedId) {
      ...
      if (removed.error) console.error("[manage-registration] cleanup of unclaimed auth user failed", removed.error);
    }
    throw error;
  }
  try {
...
async function reject(application: Application, adminId: string, reason: string): Promise<{ warning?: string }> {
  if (application.status !== "pending") throw new Error("Registration is no longer pending");
  await dbResult("reject bot registration", adminClient().rpc("reject_bot_registration", {
    _registration_id: application.id,
    _admin_id: adminId,
    _reason: reason,
  }));
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const application = await dbResult(
      "load application",
      adminClient().from("pending_registrations").select("*").eq("id", action.registrationId).single(),
    );
    if (!application) throw new Error("Application not found");
    let result: { warning?: string };
    if (action.action === "approve") result = await approve(application, adminId);
    else if (action.action === "reject") result = await reject(application, adminId, action.reason ?? "");
    else {
      if (application.status !== "approved" || application.invite_sent) throw new Error("Nothing to resend");
      try {
        await deliverWelcome(application);
        result = {};
      } catch (error) {
    ...
    return jsonResponse({ success: false, error: message }, message === "Unauthorized" || message === "Admins only" ? 403 : 400);
  }
});
