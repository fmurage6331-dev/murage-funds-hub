...
function canNotify(member: Profile | null): member is Profile & { phone_number: string } {
  return Boolean(member?.phone_number && member.status === "approved" &&
    !member.is_anonymized && member.whatsapp_opt_in);
}
...
async function memberById(id: string): Promise<Profile | null> {
  return dbResult("notification member", adminClient().from("profiles").select("*").eq("id", id).maybeSingle());
}

async function claimEvent(key: string): Promise<boolean> {
  try {
    const { error } = await adminClient().from("bot_notification_events").insert({ event_key: key });
...
async function deliverOnce(key: string, member: Profile & { phone_number: string }, message: string): Promise<boolean> {
  ...
  try {
    ...
    try {
      await dbResult("release failed notification", adminClient().from("bot_notification_events").delete().eq("event_key", key));
    } catch (releaseError) {
...
export async function notifyContributionConfirmed(id: string): Promise<boolean> {
  const row = await dbResult(
    ...
    adminClient().from("contributions").select("id,member_id,amount,status,reference,mpesa_transaction_id,confirmed_at")
      .eq("id", id).maybeSingle(),
  );
  if (!row || row.status !== "confirmed") return false;
  const member = await memberById(row.member_id);
  if (!canNotify(member)) return false;
  const total = await dbResult("confirmed total for delivery", adminClient().rpc("bot_confirmed_total", { _member_id: member.id }));
...
export async function notifyLoanDecision(id: string): Promise<boolean> {
  const loan = await dbResult(
    ...
    adminClient().from("loans").select("id,member_id,amount,loan_type,status,rejection_reason")
      .eq("id", id).maybeSingle(),
  );
  if (!loan || (loan.status !== "approved" && loan.status !== "rejected")) return false;
  const member = await memberById(loan.member_id);
  if (!canNotify(member)) return false;
  const message = loan.status === "approved"
...
export async function notifyMeetingScheduled(id: string): Promise<{ sent: number; failed: number }> {
  const meeting = await dbResult(
    "meeting for delivery",
    adminClient().from("meetings").select("id,title,scheduled_for,location,agenda").eq("id", id).single(),
  );
  ...
  for (let offset = 0; ; offset += 100) {
    const members = await dbResult(
      ...
      adminClient().from("profiles").select("*").eq("status", "approved")
        .eq("is_anonymized", false).eq("whatsapp_opt_in", true)
        .not("phone_number", "is", null).order("id").range(offset, offset + 99),
    );
    ...
    for (let start = 0; start < members.length; start += 5) {
      ...
      const results = await Promise.allSettled(batch.map((member) =>
        deliverOnce(`meeting:${meeting.id}:${member.id}`, member, message)));
