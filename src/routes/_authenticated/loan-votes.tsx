...
import { sendWhatsAppSMS } from "@/lib/notifications";
  ...
  const vote = useMutation({
    mutationFn: async ({ loan_id, v }: { loan_id: string; v: "approve" | "reject" }) => {
      const { error } = await supabase.from("loan_votes").insert({
        loan_id,
        board_member_id: user.id,
        vote: v,
        comment: comments[loan_id] || null,
      });
      if (error) throw error;
      // Board-majority trigger may change the loan status during this vote.
      ...
      if (decision?.status === "approved" || decision?.status === "rejected") {
        return sendWhatsAppSMS({
          type: decision.status === "approved" ? "loan_approved" : "loan_rejected",
          recordId: loan_id,
        });
      }
      return null;
    ...
    onSuccess: (delivery) => {
      ...
      if (delivery && !delivery.success)
        toast.warning("Decision saved, but WhatsApp/SMS delivery failed. Check the Edge logs.");
      qc.invalidateQueries({ queryKey: ["board-loans"] });
    },
    ...
    <div className="mx-auto max-w-4xl space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Forwarded loan requests awaiting board decision. Majority of board members approves.
        </p>
      </div>
            ...
            <Card key={l.id} className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    Repayment: {l.repayment_months} months
                  </div>
                  <div className="mt-1 text-xs">
                    ...
                    {l.auto_eligible ? (
                      ...
                      <span className="text-destructive">{l.eligibility_note}</span>
                    )}
                  </div>
                </div>
                <div className="text-right text-xs text-muted-foreground">
                  <div>
                    Approve: <span className="font-medium text-success">{approves}</span>
                  </div>
                  <div>
                    Reject: <span className="font-medium text-destructive">{rejects}</span>
                  </div>
                </div>
              </div>
              ...
              {l.status === "forwarded" && r.isBoard && !myVote && (
                <div className="mt-4 space-y-2 border-t border-border pt-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => vote.mutate({ loan_id: l.id, v: "approve" })}>
                      Approve
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => vote.mutate({ loan_id: l.id, v: "reject" })}
                    >
                      Reject
                    </Button>
                  </div>
              ...
              {myVote && (
                <div className="mt-3 text-xs text-muted-foreground">
                  You voted{" "}
                  <span className="font-medium capitalize text-foreground">{myVote.vote}</span>
