    ...
    onSuccess: (_result, variables) => {
      ...
      void (async () => {
        const { data: decided, error } = await supabase
          .from("loans")
          .select(
            "id,amount,loan_type,status,rejection_reason,repayment_months,profiles:member_id(email,full_name)",
          )
          .eq("id", variables.loan_id)
          .single();
        if (error) throw error;
        ...
        console.error("Failed sending loan decision notification", notificationError),
      );
    },
