...
async function reviewRegistration(action: ReviewAction): Promise<ReviewResponse> {
  ...
  if (!isReviewResponse(response) || !response.success) {
    throw new Error(
      isReviewResponse(response) ? (response.error ?? "Review failed") : "Invalid server response",
    );
  }
  return response;
}
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Review phone-only and email members. Only approved email applicants receive a password
          setup link.
        </p>
      </div>
      {error && (
            ...
            {isLoading ? (
              ...
              data.pending.map((application) => (
                <TableRow key={application.id}>
                  ...
                  <TableCell>
                    {application.email ?? <Badge variant="outline">📱 Phone Only</Badge>}
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline">
                      {application.registration_channel === "sms" ? "📱 SMS" : "💬 WhatsApp"}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-1">
                      <Button
                        ...
                        onClick={() =>
                          review.mutate({ action: "approve", registrationId: application.id })
                        }
                      >
      ...
      {data.unsent.length > 0 && (
        <Card className="space-y-2 p-4">
          ...
          <p className="text-xs text-muted-foreground">
            The account is active; retry delivery after checking your Africa’s Talking settings.
          </p>
          {data.unsent.map((application) => (
            ...
            >
              <span>
                {application.full_name} · +{application.phone_number}
              </span>
              <Button
                size="sm"
                variant="outline"
      ...
      <Dialog open={rejecting !== null} onOpenChange={(open) => !open && setRejecting(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              The reason will be sent to {rejecting?.full_name} by WhatsApp or SMS.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            aria-label="Reason for rejection"
            placeholder="Please explain your decision (at least 5 characters)"
            value={reason}
